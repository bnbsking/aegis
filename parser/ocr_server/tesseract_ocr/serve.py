...
from tesseract_ocr.exceptions import BaseCustomException
from tesseract_ocr.logs import setup_logging
...
@app.post("/run_img_list")
def run_img_list(
        files: list[UploadFile] = File(...),
        meta: UploadRequest = Depends(UploadRequest.as_form)
    ) -> List[str]:
    img_bytes_list = [f.file.read() for f in files]
    out = ocr.run_img_list(img_bytes_list)
    return out
